...
console.log("CORS options configured for local development");
...
// Debug endpoint to list all vouchers
app.get("/api/debug/vouchers", async (req, res) => {
	try {
		if (!db) {
			return res.status(500).json({ error: "Database connection error" });
		}
...
// Start the server (HTTP for local development)
async function startServer() {
	try {
		...
		app.listen(PORT, "localhost", () => {
			...
			console.log(`Debug endpoint: http://localhost:${PORT}/api/debug/vouchers`);
